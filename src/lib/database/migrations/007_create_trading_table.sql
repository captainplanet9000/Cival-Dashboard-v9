...
-- Add RLS policies to trading tables
...
CREATE POLICY "Allow authenticated users to view trading orders" ON trading_orders
    FOR SELECT USING (auth.role() = 'authenticated');

-- Create RLS policies for trading_positions
CREATE POLICY "Allow authenticated users to view trading positions" ON trading_positions
    FOR SELECT USING (auth.role() = 'authenticated');

-- Create RLS policies for trading_signals
CREATE POLICY "Allow authenticated users to view trading signals" ON trading_signals
    FOR SELECT USING (auth.role() = 'authenticated');

-- Create RLS policies for market_data_snapshots
CREATE POLICY "Allow authenticated users to view market data snapshots" ON market_data_snapshots
    FOR SELECT USING (auth.role() = 'authenticated');
