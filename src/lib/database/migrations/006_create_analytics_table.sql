...
-- Add RLS policies to analytics tables
ALTER TABLE analytics_snapshots ENABLE ROW LEVEL SECURITY;
ALTER TABLE performance_history ENABLE ROW LEVEL SECURITY;
ALTER TABLE risk_events ENABLE ROW LEVEL SECURITY;

-- Create RLS policies for analytics_snapshots (allowing all authenticated users to view)
CREATE POLICY "Allow authenticated users to view analytics snapshots" ON analytics_snapshots
    FOR SELECT USING (auth.role() = 'authenticated');

-- Create RLS policies for performance_history
CREATE POLICY "Allow authenticated users to view performance history" ON performance_history
    FOR SELECT USING (auth.role() = 'authenticated');

-- Create RLS policies for risk_events
CREATE POLICY "Allow authenticated users to view risk events" ON risk_events
    FOR SELECT USING (auth.role() = 'authenticated');
