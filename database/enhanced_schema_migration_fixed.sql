-- Enhanced Database Schema Migration - FIXED VERSION
-- This migration adds all missing tables with corrected foreign key references
-- Created: 2025-01-02
...
-- Cross-chain operations for multi-chain farms (using actual farm column)
CREATE TABLE IF NOT EXISTS public.cross_chain_operations (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    farm_id UUID, -- Will reference farms table, but make it optional for now
...
-- Blockchain achievements for goal verification (using actual goal column)
CREATE TABLE IF NOT EXISTS public.blockchain_achievements (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    goal_id UUID, -- Will reference goals table, but make it optional for now
    agent_id UUID REFERENCES public.agents(id) ON DELETE CASCADE,
    ...
    -- Constraints (removed unique constraint for now)
    CONSTRAINT unique_goal_achievement_type UNIQUE(goal_id, achievement_type)
);
...
-- ===============================================
-- 7. ADD FOREIGN KEYS AFTER CHECKING EXISTING TABLES
-- ===============================================

-- These will be added after we confirm the actual column names
-- ALTER TABLE public.cross_chain_operations 
-- ADD CONSTRAINT fk_cross_chain_farm 
-- FOREIGN KEY (farm_id) REFERENCES public.farms(farm_id) ON DELETE CASCADE;

-- ALTER TABLE public.blockchain_achievements 
-- ADD CONSTRAINT fk_blockchain_goal 
-- FOREIGN KEY (goal_id) REFERENCES public.goals(goal_id) ON DELETE CASCADE;
...
-- ===============================================
-- 9. TRIGGER FUNCTIONS FOR AUTO-UPDATES
-- ===============================================
