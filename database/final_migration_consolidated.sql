-- FINAL CONSOLIDATED DATABASE MIGRATION - Handles Existing Objects
-- This script safely creates all missing tables and handles existing triggers/functions
-- Created: 2025-01-02 - Final version with conflict resolution
...
-- Cross-chain operations (no foreign key constraints for flexibility)
CREATE TABLE IF NOT EXISTS public.cross_chain_operations (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    farm_id UUID, -- Optional reference to farms table
...
-- Blockchain achievements (no foreign key constraints for flexibility)
CREATE TABLE IF NOT EXISTS public.blockchain_achievements (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    goal_id UUID, -- Optional reference to goals table
    agent_id UUID REFERENCES public.agents(id) ON DELETE CASCADE,
...
DROP TRIGGER IF EXISTS update_system_configuration_updated_at ON public.system_configuration;
...
SELECT 'Final consolidated database schema migration completed successfully!' as status;